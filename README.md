...
### 1. 跟 Unity 有關 ###
   ...
   * #### 利用 Unity 框架做出 Real-Time 的以一個二維平面將任何形狀的三維物體切成一半 (完全是自己想的方法) ####
     ...
     2. 一個面會有 3 個點，如果其中有 1 個點、2 個點在二維平面法線方向以外，重新編輯面的資訊，使截環出現 (此時截面是中空的所以要想方法組合截面)。
     4. 判斷邊線的資訊，並且記錄順序形成一個環，或多個環 (環一定為固定方向、這跟面的資訊-法向量-有關)。
     5. 將這個還分解，使其能夠組合成面，這裡必須處裡很多特殊狀況 (甜甜圈截面、特殊拓譜截面)。
     6. 組合面，並且處理特殊狀況 (螺旋截面)。
